...
try:
    from orderflow.core.exceptions import SessionTypeAbsent
    ...
    from orderflow.core.exceptions import SessionTypeAbsent
    from orderflow.core.configuration import *
    from orderflow.market.profiles.volume_profile_kde import gaussian_kde, gaussian_kde_numba_parallel, get_kde_high_low_price_peaks, gaussian_kde_sliding_window
...
def get_volume_profile_peaks_valleys(data: pd.DataFrame, tick_size: float = 0.25, fast: bool = False) -> np.array:
    ...
    for i in tqdm(range(1, len_ - 1)):
        ...
        if np.any(peaks_indexes):
            ...
            if peaks_prices[0] != price[i] and peaks_prices[-1] != price[i]:
                if peaks_prices[curr_price_position] == price[i] and peaks_volumes[curr_price_position] < peaks_volumes[
