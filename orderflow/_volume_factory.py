...
def prepare_data(data:pd.DataFrame) -> pd.DataFrame:
    ...
    data = (
            data.
            assign(Index       = np.arange(0, data.shape[0], 1)).
...
def filter_big_prints_on_ask(data:pd.DataFrame, volume_filter:int=100) -> pd.DataFrame:
    ...
    filtered_on_ask = (
                       data.
                       query('TradeType == 2').
                       query('Volume >= ' + str(volume_filter) + '')
                      )

    return filtered_on_ask


def filter_big_prints_on_bid(data: pd.DataFrame, volume_filter: int=100) -> pd.DataFrame:
